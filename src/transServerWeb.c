...
//gcc -Wall transServerWeb.c -o transServerWeb -lpthread -I/seng/seng462/group2/seng462DB/include -L/seng/seng462/group2/seng462DB/lib -lpq -O2
...
typedef struct webSock_queue
{
	struct webSock_queue_element *start;
	struct webSock_queue_element *end;
	sem_t *length;
	pthread_mutex_t *busy;
} webSock_queue;

typedef struct webSock_queue_element
{
	long int *webSock;
	struct webSock_queue_element *next;
} webSock_queue_element;

struct conn_args
{
	struct webSock_queue *webSockList;
...
webSock_queue_element *wenq(webSock_queue *q, long int *webSock);
long int *wdeq(webSock_queue *q);
...
int main( int argc , char *argv[])
{
	...
	webSock_queue *webSockList = calloc(1, sizeof(webSock_queue));
	webSockList->start = calloc(1, sizeof(webSock_queue_element));
	webSockList->end = webSockList->start;
	webSockList->busy = malloc(sizeof(pthread_mutex_t));
	if (pthread_mutex_init(webSockList->busy, NULL))
	{
		perror("mutex init failed");
		return 1;
	}
	webSockList->length = malloc(sizeof(sem_t));
	if (sem_init(webSockList->length, 0, 0))
	{
		perror("semaphore init failed");
		return 1;
	}
...
/*******************************************************************/
	...
	struct conn_args connArgs = {webSockList, dbReadList, dbWriteList, logList, uTree, uTreeBusy, uList, sList, sTree, sTreeBusy};
	for(i=0;i<30;i++)
	{
		pthread_t connection_thread;
		if( pthread_create( &connection_thread , NULL ,  connection_handler , (void*) &connArgs) < 0)
		{
			perror("could not create thread");
			return 1;
		}
...
void *connection_handler(void *connArgs)
{
	...
	webSock_queue *wq = ((struct conn_args *)connArgs)->webSockList;
	...
	{
		webSock = wdeq(wq);
		if( (readSize = recv(*webSock, clientMessage, 200, 0)) < 1 )
		{
			close(*webSock);
			free(webSock);
			continue;
		...
		else if (!strcmp("WSC", message[0]))
		{
			...
			else
			{
				...
				cenq(cUser->commList, transID, userCmd, message[4], webSock, uList);
			}
		}
	}
...
{
	pthread_mutex_lock(q->busy);
	q->end->webSock = webSock;
	webSock_queue_element *element = q->end;
	q->end->next = calloc(1, sizeof(webSock_queue_element));
	q->end = q->end->next;
	sem_post(q->length);
	pthread_mutex_unlock(q->busy);
	return element;
...
long int *wdeq(webSock_queue *q)
{
	...
	long int *webSock = q->start->webSock;
	webSock_queue_element *element = q->start;
	q->start = q->start->next;
	free(element);
	pthread_mutex_unlock(q->busy);
	return webSock;
